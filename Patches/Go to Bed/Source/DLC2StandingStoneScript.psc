...
function WorkOnPillar(bool bSleepMove = true)
	if GetLinkedRef()
		...
		; Moving player to furniture directly will cause inability to move if player was in furniture state before, was sleeping in bed, for example.
		; Move player to the stone first.
		Game.GetPlayer().MoveTo(self)
		; Rotate player to avoid the stone activation text to pop up.
		Game.GetPlayer().SetAngle(0.0, 0.0, 180.0)
		; Move player to furniture and activate it.
